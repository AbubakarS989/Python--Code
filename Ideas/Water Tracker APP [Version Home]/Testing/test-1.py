...
class WaterTracker:
    ...
    def read_json(self):
        try:
            ...
            return {}  # Initialize with an empty dictionary if file does not exist
    ...
    def calculate_previous_dues(self):
        try:
            if self.data:
                last_entry_id = max(self.data.keys(), key=int)  # Get the most recent ID
    ...
    def get_next_id(self):
        """Determine the next ID to use for new entries."""
        if self.data:
            last_id = max(self.data.keys(), key=int)
            return int(last_id) + 1
        return 1  # Start with 1 if no entries exist
    ...
    def handle_no_purchase(self):
        try:
            ...
            current_date = datetime.now().strftime("%d-%m-%YY")
            new_entry = [
                ["Present"],
                [{
                    "Date": current_date,
                ...
                [{
                    ...
                    "Total Drum": 0,
                    "Grand_Total_Bill": 0,
                    "Grand_Total_Paid": today_paid_bill,
                    "Grand_Total_Dues": today_dues,
                }]
            ]
        ...
        try:
            ...
            new_entry = [
                ...
                [{
                    ...
                    "Grand_Total_Paid": today_paid_bill,
                    "Grand_Total_Dues": self.previous_dues,
                }]
            ]
            self.data[str(self.next_id)] = new_entry
            self.next_id += 1
