...
class WaterTracker:
    ...
    def calculate_previous_dues(self):
        try:
            if self.data:
                ...
                if len(last_entry) > 3:
                    previous_dues = last_entry[3][0].get("Today Dues", 0)
    ...
    def handle_no_purchase(self):
        try:
            ...
            payment_option = input("Choose payment option: \n1: Pay Previous Dues\n2: No Payment\nYour choice: ").strip()
            ...
            new_entry = [
                ...
                [{
                    "Monthly Cans": 0,
                    "Total Cans": self.get_total_quantity("Total Cans"),
                    "Monthly Coolers": 0,
                    "Total Coolers": self.get_total_quantity("Total Coolers"),
                    "Monthly Drum": 0,
                    "Total Drum": self.get_total_quantity("Total Drum"),
                    "Grand_Total_Bill": self.get_grand_total("Grand_Total_Bill"),
        ...
        try:
            ...
            new_entry = [
                ...
                [{
                    ...
                    "Total Cans": self.get_total_quantity("Total Cans") + quantity_cans,
                    "Monthly Coolers": quantity_coolers,
                    "Total Coolers": self.get_total_quantity("Total Coolers") + quantity_coolers,
                    "Monthly Drum": quantity_drums,
                    "Total Drum": self.get_total_quantity("Total Drum") + quantity_drums,
    ...
    def get_total_quantity(self, key):
        """Retrieve the total quantity value from the most recent entry."""
        if self.data:
            last_entry_id = max(self.data.keys(), key=int)
            last_entry = self.data[str(last_entry_id)]
            if len(last_entry) > 4:
