...
4: Exit.""")
        ...
        try:
            ...
            if ask==1:
                ...
                color_code = input("""Select your graph color:
1: Green
2: Red
3: Blue
4: Yellow
...
Enter Value: """)
        ...
        if plain_text == "Success":
            print(f"\n--------------  Your Value is Updated {plain_text}fully. -----------------\n")
            print("-------------------         Code With Abubakar        ----------------------\n")
        else:
            print("Failed to Update  your Account")
    ...
    def delete_graph(self, username, token):
        ...
        if ask == "yes":
            try:
                r = requests.delete(url=del_graph_endpoint,
                                    headers=self.header)
                print(r.text)
                data = r.text
                plain_text = self.word_check(data)
                if plain_text == "Success":
                    print(
                        f"--------------  Your Graph is Deleted {plain_text}fully. -------------------\n")
                    print(
                        "------------               Code With Abubakar                -----------------\n")
                else:
